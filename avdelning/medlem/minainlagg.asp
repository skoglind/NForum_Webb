...
<%
  ...
  If anvID = 0 Then Response.Redirect("default.asp")
  ...
  If Not config_LockDown_Forum Then
    RS_Open 1, "SELECT tID, tAmne, tTextM, tDatum_Skapad, tStatus_Trad, tStatus_UnderTrad, " & _
               ...
               "WHERE tDatum_Skapad <= '" & Now & "' AND (fSec_View = '0' OR fSec_View LIKE '%;" & SEC_TITEL & ";%') AND tForum <> " & CLng(config_Trashbin) & " AND tStatus_Raderad = 0 AND tAnv_Skapad = " & CLng(anvID) & " ORDER BY tDatum_Skapad DESC", False
...
<%
  ...
  page_Title    = "Mina inlägg - [Sida " & iPageData & "] - Medlem"
  page_Header   = lMedlem & "s inlägg"
    ...
    <div class="nf_datablock nf_size_full">
      ...
      <% If any_Tradar Then %>
        ...
        <div class="nf_paging nf_paging_full">
          <a href="minainlagg.asp?m=<% = lMedlem %>&amp;page=<% = pagingOnPage-1 %>">««</a> |
          
          <% For Each zx In pagingPages %>
            <% If zx = "..." Then %>
              ... |
            <% Else %>
              <a href="minainlagg.asp?m=<% = lMedlem %>&amp;page=<% = zx %>" <% If CLng(zx) = CLng(pagingOnPage) Then Response.Write(" class='c'") %>><% = zx %></a> <% If CLng(zx) < pagingNumOfPages Then %> | <% End If %>
            <% End If %>
          <% Next %>
