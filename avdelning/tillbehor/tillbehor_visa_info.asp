...
<%
  ...
  RS_Open 1, "SELECT *, " & _
    ...
    If text_Img4 > 0 Then text_UseArt = text_Img4 : text_UseText = "Tillbehör"
...
<%
  ' ## Globala variabler ##
  page_Title    = text_Titel & " - Information - Tillbehör"
  page_Header   = text_Titel
  page_WhereAmI = "&gt; <a href='default.asp' title='Gå till &quot;Tillbehör&quot; ...'>Tillbehör</a> " & _
...
<!--#INCLUDE FILE="../../_page_middle.asp"-->
  ...
  <div class="content">
    ...
    <div class="nf_datablock nf_size_full">
      <h1><span class="nf_extitel"><a href="/avdelning/tillbehor/">Tillbehör</a></span><img src="<% = config_GFXLocation %>icons/flags/<% = text_Region %>.png" alt="" title=""> <% = text_Titel %></h1>
    ...
    <div class="nf_datablock nf_size_twothird">
        ...
        <div class="nf_images">
          ...
          <% If text_Img4 > 0 Then %>
            <% If CONST_LOGIN Then %><a href="<% = config_ImageLocation %>?e=<% = text_Img4 %>&amp;w=640&amp;h=480" title="Kassett/Media" target="_blank" rel="lightbox[boxart]"><% End If %>
              <img class="boxart" src="<% = config_ImageLocation %>?e=<% = text_Img4 %>&amp;w=100&amp;h=100" title="Tilbehör" alt="Konsol">
            <% If CONST_LOGIN Then %></a><% End If %>
          <% End If %>
      ...
      <div class="nf_text">
        ...
        <% If Len(Trim(text_LargeText)) > 0 Then %>
          <p><% = text_LargeText %></p>
        <% Else %>
          <p><% = text_Titel %> är ett tillbehör som tillhör konsolfamiljen <strong><a href="default.asp?k=<% = text_KonsolID %>"><% = text_Konsol %></a></strong>. Denna konsol är utgiven <strong><% = text_Release %></strong>.</p>
          <p>Vi har tyvärr ingen mer information om detta tillbehör.</p>
        <% End If %>
      </div>
      ...
      <% If CONST_LOGIN Then %>
        ...
        <div class="nf_msg nf_green">
          <p><strong>Du har listat följande kopior av tillbehöret...</strong></p>
          <ul class="nf_rowlist" id="titleListed_List" style="<% If Not any_Samling Then Response.Write("display: none;") %>"></ul>   
          <p class="nf_pretend_rowlist" id="titleListed_Mess" style="<% If any_Samling Then Response.Write("display: none;") %>">Du har inte listat tillbehöret.</p>
          <p style="text-align: center;"><input style="float: none;" type="button" onclick="OpenCollection('addon',<% = lID %>,0,'new');" value="Lägg till i samlingen"></p>
          <% If any_SomHar Then %>
              ...
              <% For zx = 0 To UBound(list_SomHar, 2) %>
                <li>
                  ...
                  <div class="nf_collectionbar" style="background-image: url('<% = config_GFXLocation %>icons/samling/samling_alla_tillbehor.png');">
                    ...
                    <img alt="" title="Tillbehör" src="<% = config_GFXLocation %>icons/samling/no<% If list_SomHar(4,zx) Then Response.Write("blank") %>.png">
      ...
      <% Else %>
        <% ' #### SAMLINGEN #### %>
        <div class="nf_msg nf_green">
          <p style="text-align: center;">Du måste <strong><a href="/avdelning/medlem/loggain.asp">logga in</a></strong> för att kunna lista dina tillbehör.</p>
